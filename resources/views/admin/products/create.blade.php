        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                    @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">Slug</label>
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm font-mono" placeholder="otomatis dari judul">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="6" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">{{ old('description') }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-blue-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description_en" id="description_en" rows="6" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="English description...">{{ old('description_en') }}</textarea>
                    @error('description_en') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div x-data="dynamicList('features_en', @js(old('features_en', [''])))">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" step="1000" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="0">
                        @error('price') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="version" class="block text-sm font-medium text-gray-700 mb-1">Versi</label>
                        <input type="text" name="version" id="version" value="{{ old('version') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="contoh: 1.0.0">
                ...
                <div>
                    ...
                    <input type="url" name="demo_url" id="demo_url" value="{{ old('demo_url') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="https://demo.example.com">
                    @error('demo_url') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                {{-- Dynamic Tech Stack --}}
                <div x-data="dynamicList('tech_stack', @js(old('tech_stack', [''])))">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tech Stack</label>
                    <template x-for="(item, index) in items" :key="index">
                        <div class="flex gap-2 mb-2">
                            <input type="text" :name="fieldName + '[' + index + ']'" x-model="items[index]" class="flex-1 rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="contoh: Laravel 12">
                            <button type="button" @click="remove(index)" class="px-2 py-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors" x-show="items.length > 1">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                ...
                {{-- Dynamic Features --}}
                <div x-data="dynamicList('features', @js(old('features', [''])))">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Fitur Utama</label>
                    <template x-for="(item, index) in items" :key="index">
                        <div class="flex gap-2 mb-2">
                            <input type="text" :name="fieldName + '[' + index + ']'" x-model="items[index]" class="flex-1 rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="contoh: Multi-role user management">
                            <button type="button" @click="remove(index)" class="px-2 py-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors" x-show="items.length > 1">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                </svg>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="file" name="thumbnail" id="thumbnail" accept="image/*" required class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                    <p class="text-xs text-gray-400 mt-1">JPG, PNG, atau WebP. Maks 5MB. Akan di-resize 800x600px.</p>
                    @error('thumbnail') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="file_path" class="block text-sm font-medium text-gray-700 mb-1">File Source Code (ZIP/RAR)</label>
                    ...
                    <p class="text-xs text-gray-400 mt-1">ZIP atau RAR. Maks 100MB. Disimpan di private storage.</p>
                    @error('file_path') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="images" class="block text-sm font-medium text-gray-700 mb-1">Screenshot/Preview</label>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" maxlength="255">
                    @error('meta_title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="meta_description" class="block text-sm font-medium text-gray-700 mb-1">Meta Description</label>
                    <textarea name="meta_description" id="meta_description" rows="2" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" maxlength="500">{{ old('meta_description') }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" name="meta_title_en" id="meta_title_en" value="{{ old('meta_title_en') }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" maxlength="255" placeholder="English meta title...">
                ...
                <div>
                    ...
                    <textarea name="meta_description_en" id="meta_description_en" rows="2" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm" maxlength="500" placeholder="English meta description...">{{ old('meta_description_en') }}</textarea>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="flex-1 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">Simpan</button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <p class="text-sm text-gray-400 mt-2"><a href="{{ route('admin.product-categories.create') }}" class="text-indigo-600 hover:underline">Buat kategori</a> terlebih dahulu.</p>
                @endif
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <label class="flex items-center gap-2 py-1.5">
                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span class="text-sm text-gray-700">{{ $tag->name }}</span>
                </label>
                @empty
                <p class="text-sm text-gray-400">Belum ada tag. <a href="{{ route('admin.product-tags.create') }}" class="text-indigo-600 hover:underline">Buat tag</a></p>
                @endforelse
            </div>
        </div>
    </div>
