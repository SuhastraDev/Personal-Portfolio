...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900">Edit Kategori: {{ $portfolioCategory->name }}</h1>
</div>
    ...
    <form action="{{ route('admin.portfolio-categories.update', $portfolioCategory) }}" method="POST">
        ...
        <div class="p-6 space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $portfolioCategory->name) }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            ...
            <div class="border-l-4 border-blue-400 pl-4">
                ...
                <input type="text" name="name_en" id="name_en" value="{{ old('name_en', $portfolioCategory->name_en) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm" placeholder="e.g. Web Development">
                @error('name_en') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="slug" class="block text-sm font-medium text-gray-700 mb-1">Slug</label>
                <input type="text" name="slug" id="slug" value="{{ old('slug', $portfolioCategory->slug) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm font-mono">
